...
function ProductList() {
  ...
  async function deleteProductByID(product_id, title) {
    try {
      ...
      setProduct(products.filter((product) => product.id !== product_id))
        ...
        {products
          .filter((product) => {
            if (searchTerm == '') {
              ...
              let searchAuthor = product.author
                .toLowerCase()
                .includes(searchTerm.toLowerCase())
              if (searchTitle || searchAuthor) return product
            }
          })
          .map((product) => (
            <ContainerItem key={product.id}>
              ...
              <ContainerButtons>
                ...
                />
                <ButtonStyle
                  onClick={() => deleteProductByID(product.id, product.title)}
                >
